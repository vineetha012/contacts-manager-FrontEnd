...
export const postContacts = async (contactArr) => {
    ...
    const res = await fetch('https://cmc-server.onrender.com/v1/contacts', {
        method: "POST",
        headers: {
...
export const getContacts = async () => {
    try {
        const res = await axios.get(
            'https://cmc-server.onrender.com/v1/contacts',
            {headers: 
                {'Authorization': localStorage.getItem('token')}
            })
...
export default function UseSession() {
    ...
    return new Promise ((resolve, reject) => {
        ...
        return fetch(`https://cmc-server.onrender.com/v1/signin/get-current-user?sessionId=${sessionId}`)
            .then(res => res.json())
            .then(data => {
                if(data) {
                    resolve(data);
